...
export async function exportToPDF(data: AnalyzeResponse): Promise<void> {
  ...
  doc.text('Review sentiment mix', marginX, barTop - 4)
  ...
  doc.text(leftFooter, marginX, y)

  const tealRgb = hexRgb('#2DD4BF')
  doc.setTextColor(tealRgb.r, tealRgb.g, tealRgb.b)
  doc.text(footerUrl, pageW - marginX - doc.getTextWidth(footerUrl), y)

  const pageCount = doc.getNumberOfPages ? doc.getNumberOfPages() : 1
  doc.setTextColor(100, 116, 139)
  doc.text(`Page ${pageCount}`, pageW / 2 - doc.getTextWidth(`Page ${pageCount}`) / 2, y)
