import Link from 'next/link'
import { ArrowLeft } from 'lucide-react'

export default function ReviewsPage() {
  return (
    <div className="p-6">
      <div className="flex items-center gap-4 mb-6">
        <Link href="/dashboard" className="text-text-secondary hover:text-text-primary">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <h1 className="text-2xl font-medium text-text-primary">Review Analysis</h1>
      </div>
      <div className="bg-background-card border border-border rounded-xl p-8 text-center">
        <p className="text-text-secondary">Detailed review analysis coming soon...</p>
        <p className="text-text-muted text-sm mt-2">This page will show in-depth review breakdowns and sentiment analysis.</p>
      </div>
    </div>
  )
}
